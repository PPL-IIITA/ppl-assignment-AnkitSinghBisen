...
import java.util.Date;
        ...
        /**
	 * handles Commitments, Gift exchange and breakups.
    ...
    public static int CommitandGift(boy[] boys, girl[] girls, gift[] gifts, int allocations[][], int b, int g, int gif, String Logdata[][]) {
        ...
        for (i = 0; i < g; i++) {
            ...
            for (j = 0; j < b; j++) {
                ...
                if ((boys[j].budget > girls[i].Maintbudget) && (girls[i].attractiveness > boys[j].minatreq) && (boys[j].relstat == 's') && (girls[i].exboyfriend !=j)) {
